...
public class SpaceGravityReal : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        for (int b = 0; b < objs.Length; b++)
        {
            ...
            if (objs[b].GetComponent<SpaceTrajectory>() != null)
            {
                objs[b].GetComponent<SpaceTrajectory>().m2Pos = (Vector3d)SOIm2Pos;
                objs[b].GetComponent<SpaceTrajectory>().m2Vel = (Vector2d)SOIm2Vel;
                objs[b].GetComponent<SpaceTrajectory>().m2 = SOIm2;
                objs[b].GetComponent<SpaceTrajectory>().distance = (Vector3d)SOIm2r;
            }
