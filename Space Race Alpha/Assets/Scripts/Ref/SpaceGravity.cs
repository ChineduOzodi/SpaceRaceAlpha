...
public class SpaceGravity : MonoBehaviour {
	...
	void Update () {
        ...
        for (int b = 0; b < objs.Length; b++)
        {
            ...
            objs[b].GetComponent<SpaceTrajectory>().m2Pos = (Vector3d) m2Pos;
            objs[b].GetComponent<SpaceTrajectory>().m2Vel = (Vector2d) objs[m2PosInd].GetComponent<Rigidbody2D>().velocity;
            objs[b].GetComponent<SpaceTrajectory>().m2 = m2;
            objs[b].GetComponent<SpaceTrajectory>().distance = (Vector3d) distance;
